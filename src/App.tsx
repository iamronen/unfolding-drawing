...
} from './lib/circularRepeaterRotate';
import {
  getRelatedPlaceIds,
...
import { mirrorEchoCircularRepeaterFirstAxisLocalAngle } from './lib/mirrorEchoGeometry';
import {
  isMirrorOnlyEchoPlace,
  mirrorEchoPlaceAngles,
} from './lib/mirrorEchoGeometry.ts';
import {
  getEchoesInMirrorAxisOrder,
  getMirrorAxisIdForEchoPlace,
...
/** Default radians for new axes (relative to parent place) and circle angles; screen “north” (up), not math +x (east). */
const DEFAULT_DRAWING_ORIENTATION_RAD = -Math.PI / 2;
...
const App: Component = () => {
      ...
      onToggleBendAtA: () => {
        ...
        if (bendAtA) {
          ...
          const deletedEntries = toRemove
            .map((b) => {
              const placeIdA = ends.find(
                (e) => e.id === b.lineSegmentEndId,
              )?.placeId;
  ...
  const [draggingCircularFieldRadius, setDraggingCircularFieldRadius] =
    createSignal<CircularFieldId | null>(null);
  const [radiusHandleDragPosition, setRadiusHandleDragPosition] = createSignal<{
    x: number;
    y: number;
  } | null>(null);
  ...
  type PendingAddLineSegment =
    | {
        mode: 'circularRepeater';
  ...
  const [pendingAddLineSegment, setPendingAddLineSegment] =
    createSignal<PendingAddLineSegment | null>(null);
  ...
  const getAbsolutePosition = (
    ...
    if (place.parentAxisId != null) {
      ...
      if (axis && parent) {
        ...
        const baseAxisAngle = parentRes.worldAngle + Number(axis.angle);
        const worldAngle =
  ...
  function buildMoveOverrideForPendingMove(
    ...
    const movedPlace = placesList.find((p) => p.id === pm.placeId) as
      | PlaceLike
      | undefined;
    if (!movedPlace) return { placeId: pm.placeId, x: pm.x, y: pm.y };
    const groupId = movedPlace.repeaterEchoGroupId;
    if (groupId == null) return { placeId: pm.placeId, x: pm.x, y: pm.y };
    ...
    const movedAxisAngle = getAxisWorldAngle(
      movedPlace,
      placesList,
      angleOverride,
    );
    ...
    for (const place of groupPlaces) {
      ...
      let worldDx = dx;
      let worldDy = dy;
      const placeAxisAngle = getAxisWorldAngle(
        place,
        placesList,
        angleOverride,
      );
  ...
  function getCircularFieldEchoGroupIds(
    circularFieldId: CircularFieldId,
    placesList: ReadonlyArray<PlaceLike>,
    circularFieldsList: ReadonlyArray<{
      id: CircularFieldId;
    ...
    return circularFieldsList
      .filter((c) => c.placeId != null && echoPlaceIds.has(c.placeId))
      .map((c) => c.id);
  }
  ...
  const inAxisAlternatingPattern = (
    ...
    const step =
      (((axisNumber1Based - s + axisCount) % axisCount) + axisCount) %
      axisCount;
  ...
  const placesWithAbsolutePositions = () => {
    ...
    if (pr?.kind === 'place') {
      ...
      if (groupId != null && rotatedPlace) {
        ...
        const mirrorOnly =
          rotatedPlace != null && isMirrorOnlyEchoPlace(rotatedPlace, axes());
        for (let i = 0; i < result.length; i++) {
          const echoRow = result[i];
          if (!echoRow) continue;
          if ((echoRow as PlaceLike).repeaterEchoGroupId !== groupId) continue;
          const pUnrotated = getAbsolutePosition(
            echoRow as PlaceLike,
            pl,
            moveOverride,
            null,
          ).worldAngle;
          const effectiveDelta =
            mirrorOnly && echoRow.id !== pr.placeId ? -delta : delta;
          result[i] = {
            ...echoRow,
            absWorldAngle: pUnrotated + effectiveDelta,
        ...
        while (changed) {
          ...
          for (let i = 0; i < result.length; i++) {
            ...
            if (depId == null || !updatedIds.has(depId) || updatedIds.has(p.id))
              continue;
            ...
            if (p.parentId != null) {
              ...
              const localX = radius * Math.cos(angleOnCircle);
              const localY = radius * Math.sin(angleOnCircle);
              const rotated = rotateBy(parentAbs.absWorldAngle, localX, localY);
              absX = parentAbs.absX + rotated.x;
              absY = parentAbs.absY + rotated.y;
          ...
          while (changed) {
            ...
            for (let i = 0; i < result.length; i++) {
              ...
              if (p.parentId != null) {
                ...
                if (!ax) continue;
                const worldAngle = parentAbs.absWorldAngle + Number(ax.angle);
                let d = (p as PlaceLike).distanceAlongAxis ?? 0;
                if (ax.isBidirectional === 0) d = Math.max(0, d);
                const perp = (p as PlaceLike).distanceFromAxis ?? 0;
                const cosA = Math.cos(worldAngle);
    ...
    return fields
      .map((f) => {
        ...
        return {
          ...
          radiusHandleAngle: (f as { radiusHandleAngle?: number | null })
            .radiusHandleAngle,
        };
      })
      .filter((x): x is NonNullable<typeof x> => x != null);
  ...
  const handlePointerDown = (e: PointerEvent) => {
    ...
    if (
      ...
      if (placeRes.ok) {
        const cfRes = evolu.insert('circularField', {
          ...
          radiusHandleAngle: DEFAULT_DRAWING_ORIENTATION_RAD,
        });
        if (cfRes.ok) {
    ...
    if (gs === 'execute' && tc === 'modifyPlaceOnCircularRepeater' && hit) {
    ...
    if (gs === 'execute' && tc === 'rotate') {
      const selRep = selectedCircularRepeaterId();
      if (selRep != null) {
        const axesList = axes();
        const placesAbs = placesWithAbsolutePositions();
        const refWorld = getRepeaterReferenceAxisWorldAngle(
          selRep,
          axesList,
          placesAbs,
        );
        const repRow = circularRepeaters().find((r) => r.id === selRep);
        const hub = repRow
          ? placesAbs.find((p) => p.id === repRow.placeId)
          : null;
        if (
          refWorld != null &&
          hub != null &&
          isPointNearAxis(cx, cy, hub.absX, hub.absY, refWorld, 36, true)
        ) {
          setPendingRotate({
            kind: 'circularRepeater',
            circularRepeaterId: selRep,
            angle: refWorld,
          });
        }
      } else {
        ...
        if (selId) {
          ...
          if (place) {
            const pr0 = pendingRotate();
            const theta =
              pr0?.kind === 'place'
                ? (pr0.angle ?? place.absWorldAngle ?? 0)
    ...
    if (gs === 'execute' && tc === 'addLineSegment' && pals) {
      ...
      const axesList = axes();
      const rotateEchoList = <T,>(arr: T[], startIdx: number): T[] => {
        if (startIdx < 0 || arr.length === 0) return [...arr];
        return [...arr.slice(startIdx), ...arr.slice(0, startIdx)];
      };
      const originEchoesRaw =
        pals.mode === 'circularRepeater'
          ? getEchoesInAxisOrder(
              pals.originGroupId,
        ...
        if (sameRepeaterContext) {
          ...
          if (hitGroupId != null) {
            ...
            if (pals.mode === 'circularRepeater') {
              const originSelForHub = placesList.find(
                (p) => p.id === pals.originSelectedPlaceId,
              ) as PlaceLike | undefined;
              const hub =
                originSelForHub != null
                  ? getCircularRepeaterHubPlaceId(
                      originSelForHub,
                      placesList,
                      axesList,
              ...
              const otherHub =
                hub != null
                  ...
                  : null;
              if (otherHub != null && hitPlace != null) {
                const axisNumForPlace = (pid: PlaceId) =>
                  circularRepeaterAxisNumberForPlace(pid, placesList, axesList);
                const selAxisNum = axisNumForPlace(pals.originSelectedPlaceId);
                const hitAxisNum = axisNumForPlace(hit.id);
                const otherOrigin = originEchoesRaw.find((p) => {
                  if (
                    getCircularRepeaterHubPlaceId(p, placesList, axesList) !==
                    otherHub
                  )
                    return false;
                  return (
                ...
                const otherEnd = endingEchoesRaw.find((p) => {
                  if (
                    getCircularRepeaterHubPlaceId(p, placesList, axesList) !==
                    otherHub
                  )
                ...
                if (
                  ...
                  const originEchoesO = filterEchoesBySameRepeaterHub(
                    originEchoesRaw,
                    otherOrigin.id,
                    placesList,
                    axesList,
                  );
                  const endingEchoesO = filterEchoesBySameRepeaterHub(
                    endingEchoesRaw,
                    otherEnd.id,
                    placesList,
                    axesList,
                  ...
                  const lineNameO = nextLineSegmentName(lineSegments());
                  const segNameResultO = String1000.from(lineNameO);
                  for (const [iO, iE] of indexPairsO) {
                    const origEcho = originRotatedO[iO]!;
                    const endEcho = endRotatedO[iE]!;
                    const endARes = evolu.insert('lineSegmentEnd', {
                      placeId: origEcho.id,
                    });
                    const endBRes = evolu.insert('lineSegmentEnd', {
                    ...
                    if (endARes.ok && endBRes.ok && segNameResultO.ok) {
                      const segRes = evolu.insert('lineSegment', {
                        endAId: endARes.value.id,
                        endBId: endBRes.value.id,
                      ...
                      const placeNameA =
                        ...
                        'Place';
                      const placeNameB =
                        (endEcho as { name?: string | null }).name?.trim() ||
                        'Place';
                      const endAName = String1000.from(
                        lineSegmentEndDisplayName(lineNameO, placeNameA),
                      );
                      const endBName = String1000.from(
                        lineSegmentEndDisplayName(lineNameO, placeNameB),
                      );
                      if (endAName.ok)
                        evolu.update('lineSegmentEnd', {
                          id: endARes.value.id,
      ...
      if (isCase2AddLineSegment) {
        ...
        /** Mirror-axis pair: one child at the cursor from the selected echo; partner at the reflected world point so both sides get distinct echoes (not the same world target). */
        let localByOriginId: Map<PlaceId, { x: number; y: number }> | null =
          null;
        if (pals.mode === 'mirrorAxis' && originEchoes.length === 2) {
          const selectedOrigin =
            originEchoes.find((p) => p.id === pals.originSelectedPlaceId) ??
            originEchoes[0]!;
          const otherOrigin = originEchoes.find(
            (p) => p.id !== selectedOrigin.id,
          );
          const axis = axesList.find((a) => a.id === pals.mirrorAxisId);
          if (otherOrigin && axis) {
            const placesAbs = placesWithAbsolutePositions();
            const geom = getAxisWorldGeometry(
              {
            ...
            const reflected = reflectPointAcrossAxisLine(
              cx,
              cy,
              geom.originX,
              geom.originY,
            ...
            const absSel = getAbsolutePosition(selectedOrigin, placesList);
            const localSel = rotateBy(
              -absSel.worldAngle,
              cx - absSel.x,
              cy - absSel.y,
            );
            const absOther = getAbsolutePosition(otherOrigin, placesList);
            const localOther = rotateBy(
              -absOther.worldAngle,
              reflected.x - absOther.x,
              reflected.y - absOther.y,
            );
            localByOriginId = new Map([
              [selectedOrigin.id, localSel],
              [otherOrigin.id, localOther],
            ]);
          }
        }
        ...
        for (let i = 0; i < originEchoesCase2.length; i++) {
          ...
          const local =
            ...
            (() => {
              ...
              return rotateBy(-abs.worldAngle, worldOffset.x, worldOffset.y);
            })();
          localsFirst.push(local);
        ...
        if (
          ...
          const otherHubC2 =
            hubC2 != null
              ...
              : null;
          const hubPlace = hubC2
            ? (placesList.find((p) => p.id === hubC2) as PlaceLike | undefined)
            : undefined;
          const mirrorAxisId = (() => {
            if (hubPlace == null) return null;
            if (hubPlace.parentAxisId != null) {
              const ax = axesList.find((a) => a.id === hubPlace.parentAxisId);
              if (
                ax != null &&
                (ax as { isMirror?: number | null }).isMirror === 1
              ) {
                return hubPlace.parentAxisId;
              }
            }
            return hubC2 != null
              ? getMirrorAxisIdForEchoPlace(hubC2, placesList, axesList)
              : null;
          })();
          const mirrorAxisRow =
            mirrorAxisId != null
          ...
          const otherAnchor0 =
            otherHubC2 != null && firstAxisNum >= 0
              ? originEchoesRaw.find((p) => {
                  if (
                    getCircularRepeaterHubPlaceId(p, placesList, axesList) !==
                    otherHubC2
                  )
          ...
          if (
            ...
            const geom = getAxisWorldGeometry(
              {
                id: mirrorAxisRow.id,
                placeId: mirrorAxisRow.placeId,
                angle: Number(mirrorAxisRow.angle),
              },
              placesAbs,
            );
            for (let i = 0; i < originEchoesOther.length; i++) {
              ...
              const rot = rotateBy(
                absParent.worldAngle,
                localFirst.x,
                localFirst.y,
              );
              const worldX = absParent.x + rot.x;
              const worldY = absParent.y + rot.y;
              const reflected = reflectPointAcrossAxisLine(
                worldX,
                worldY,
                geom.originX,
              ...
              const absOtherParent = getAbsolutePosition(
                originEchoOther,
                placesList,
              );
              const localOther = rotateBy(
                -absOtherParent.worldAngle,
        ...
        if (
          pals.mode === 'circularRepeater' &&
          newEchoesOther.length > 0 &&
          newEchoesOther.length === originEchoesOther.length &&
          firstLineSegId != null
        ) {
          const lineNameO = nextLineSegmentName(lineSegments());
          const segNameResultO = String1000.from(lineNameO);
          for (
            let i = 0;
            i < originEchoesOther.length && i < newEchoesOther.length;
            i++
          ) {
            const origEcho = originEchoesOther[i]!;
            const newEcho = newEchoesOther[i]!;
            const endARes = evolu.insert('lineSegmentEnd', {
              placeId: origEcho.id,
            });
            const endBRes = evolu.insert('lineSegmentEnd', {
            ...
            if (endARes.ok && endBRes.ok && segNameResultO.ok) {
              ...
              const endBName = String1000.from(
                lineSegmentEndDisplayName(lineNameO, newEcho.name),
              );
              if (endAName.ok)
                evolu.update('lineSegmentEnd', {
                  id: endARes.value.id,
    ...
    if (pr && guideStep() === 'execute' && mode() === 'default') {
      if (pr.kind === 'place') {
        ...
        if (place) {
          ...
          setPendingRotate({ kind: 'place', placeId: pr.placeId, angle });
        }
      } else if (pr.kind === 'circularRepeater') {
        const repRow = circularRepeaters().find(
          (r) => r.id === pr.circularRepeaterId,
        );
        const hub = repRow
          ? placesWithAbsolutePositions().find((p) => p.id === repRow.placeId)
          : null;
        if (hub) {
          const { x: cx, y: cy } = screenToCanvas(e.clientX, e.clientY);
          const angle = Math.atan2(cy - hub.absY, cx - hub.absX);
          setPendingRotate({
            kind: 'circularRepeater',
            circularRepeaterId: pr.circularRepeaterId,
            angle,
          });
        }
      }
    ...
    if (choice === 'addLineSegment') {
      ...
      if (circularRepeaterId != null) {
        ...
        const axis = axesList.find((a) => a.id === axisId) as
          ...
          | undefined;
        if (!axis || axis.isMirror !== 1) return;
        setPendingAddLineSegment({
          mode: 'mirrorAxis',
          originSelectedPlaceId: placeId,
          originGroupId: groupId,
      ...
      if (placeId) {
        ...
        if (groupId != null) {
          ...
          setPendingAddAxis({
            ...
            angle: DEFAULT_DRAWING_ORIENTATION_RAD,
    ...
    if (choice === 'addPlaceOnCircularField') {
      ...
      if (cfId) {
        setPendingAddPlaceOnCircularField({
          ...
          angleOnCircle: DEFAULT_DRAWING_ORIENTATION_RAD,
        });
      }
    }
  ...
  const handleCommit = () => {
        ...
        if (parent) {
          ...
          if (groupId != null) {
            ...
            const sortedEchoes = [...echoPlaces].sort((a, b) => {
              ...
              return Number(axA?.angle ?? 0) - Number(axB?.angle ?? 0);
            });
            let firstPlaceId: PlaceId | null = null;
        ...
        if (placeOnAxis) {
          ...
          if (axis) {
            ...
            if (repId != null) {
              ...
              const projX = geom.originX + d * cosA;
              const projY = geom.originY + d * sinA;
              perp = (move.x - projX) * -sinA + (move.y - projY) * cosA;
              if (Math.abs(perp) <= AXIS_MAGNETIC_THRESHOLD) perp = 0;
            }
            ...
            const isMirrorAxis =
              repId == null &&
              (axis as { isMirror?: number | null }).isMirror === 1;
            const mirrorSameAxis = isMirrorAxis && toUpdate.length >= 2;

            if (isMirrorAxis && perp === 0 && toUpdate.length >= 2) {
              evolu.update('place', {
                id: placeId,
                distanceAlongAxis: d,
            ...
            } else if (isMirrorAxis && perp !== 0 && toUpdate.length === 1) {
              evolu.update('place', {
                id: placeId,
                distanceAlongAxis: d,
                distanceFromAxis: perp,
              ...
              const placeName =
                (place as { name?: string | null }).name ?? null;
              ...
              if (repId != null && groupId != null) {
                ...
                recordTransformation({
                  ...
                  ...(paModRep?.patternEnabled === true
                    ? {
                        alternatingShow:
                          paModRep?.alternatingShow ??
                          placeRow.alternatingShow ??
                          1,
                        alternatingSkip:
                          paModRep?.alternatingSkip ??
                          placeRow.alternatingSkip ??
                          1,
                        alternatingStart:
                          paModRep?.alternatingStart ??
                          placeRow.alternatingStart ??
                          1,
                      }
                    : {}),
    ...
    if (rot?.kind === 'circularRepeater') {
      const axesListCommit = axes();
      const placesAbsPlain = placesList.map((p) => {
        const abs = getAbsolutePosition(p, placesList, null, null);
        return {
          ...p,
          absX: abs.x,
          absY: abs.y,
          absWorldAngle: abs.worldAngle,
        };
      });
      const refBase = getRepeaterReferenceAxisWorldAngle(
        rot.circularRepeaterId,
        axesListCommit,
        placesAbsPlain,
      );
      if (refBase != null) {
        const delta = rot.angle - refBase;
        const groupIds = getCircularRepeaterEchoGroupIds(
          rot.circularRepeaterId,
          circularRepeaters(),
          placesList,
          axesListCommit,
        );
        const signs = rotationSignByRepeaterId(
          groupIds,
          rot.circularRepeaterId,
          circularRepeaters(),
          placesList,
        ...
        for (const rid of groupIds) {
          const sign = signs.get(rid) ?? 1;
          const repAxes = axesListCommit.filter(
            (a) =>
              (a as { circularRepeaterId?: CircularRepeaterId })
                .circularRepeaterId === rid,
          );
          for (const axis of repAxes) {
            const newAng = Number(axis.angle) + sign * delta;
            evolu.update('axis', { id: axis.id, angle: newAng });
          }
          const hubRow = circularRepeaters().find((r) => r.id === rid);
          if (hubRow != null) {
            recordTransformation({
              kind: 'rotate',
              placeId: hubRow.placeId,
              angle: sign * delta,
              circularRepeaterId: rid,
            });
          }
      ...
      const rotatedPlaceUnrotated =
        rotatedPlace != null
          ? getAbsolutePosition(rotatedPlace, placesList, null, null).worldAngle
          : rot.angle;
      const delta = rot.angle - rotatedPlaceUnrotated;
      const mirrorOnly =
        rotatedPlace != null &&
        isMirrorOnlyEchoPlace(rotatedPlace as PlaceLike, axes());
      for (const p of toRotate) {
        ...
        if (groupId != null) {
          ...
          const targetWorld =
            mirrorOnly && p.id !== rot.placeId
              ? pUnrotated - delta
              : pUnrotated + delta;
          const axis =
            (p as PlaceLike).parentAxisId != null
              ? axes().find((a) => a.id === (p as PlaceLike).parentAxisId)
              : null;
          const parent = axis
          ...
          if (axis && parent) {
            ...
            const baseAxisAngle = parentRes.worldAngle + Number(axis.angle);
            storedAngle = targetWorld - baseAxisAngle;
          } else {
            const d = mirrorOnly && p.id !== rot.placeId ? -delta : delta;
            storedAngle = ((p as PlaceLike).angle ?? 0) + d;
          }
        } else if ((p as PlaceLike).parentAxisId != null) {
    ...
    if (addPlaceOnAx) {
      ...
      const mirrorAngles =
        isMirror && offAxis ? mirrorEchoPlaceAngles(distanceFromAxis) : null;
      const placeRes = evolu.insert('place', {
        parentId: null,
        ...
        y: 0,
        ...(nameResult.ok && { name: nameResult.value }),
        ...(mirrorAngles != null && { angle: mirrorAngles.first }),
        isScaffolding: 1,
      });
      if (placeRes.ok) {
        if (isMirror && offAxis) {
          ...
          evolu.insert('place', {
            ...
            y: 0,
            ...(echoNameResult.ok && { name: echoNameResult.value }),
            ...(mirrorAngles != null && { angle: mirrorAngles.echo }),
            isScaffolding: 1,
            repeaterEchoGroupId: placeRes.value.id,
          });
        }
    ...
    if (addRep) {
      ...
      if (echoPlaces.length <= 1) {
        ...
        if (repRes.ok) {
          ...
          const hubPlace = placesList.find((p) => p.id === addRep.placeId);
          const firstLocal = hubPlace
            ? mirrorEchoCircularRepeaterFirstAxisLocalAngle(hubPlace, axes())
            : -Math.PI / 2;
          for (let k = 0; k < count; k++) {
            const angle = firstLocal + (2 * Math.PI * k) / count;
            evolu.insert('axis', {
              placeId: addRep.placeId,
              angle,
              isBidirectional: 0,
        ...
        for (let i = 0; i < echoPlaces.length; i++) {
          ...
          if (repRes.ok) {
            ...
            const firstLocal = mirrorEchoCircularRepeaterFirstAxisLocalAngle(
              place,
              axes(),
            );
            for (let k = 0; k < count; k++) {
    ...
    if (modRep) {
      ...
      for (const currentRepId of echoRepIds) {
        const repeater = circularRepeaters().find((r) => r.id === currentRepId);
        ...
        const hubPlaceMod = placesList.find((p) => p.id === repeater.placeId);
        const firstLocalMod = hubPlaceMod
          ? mirrorEchoCircularRepeaterFirstAxisLocalAngle(hubPlaceMod, axes())
          : -Math.PI / 2;
        ...
        if (newCount > oldCount) {
          ...
          for (let k = 0; k < sorted.length; k++) {
            ...
            assigned.add(slot);
            const angle = firstLocalMod + (2 * Math.PI * slot) / newCount;
            const ax = sorted[k];
          ...
          for (let j = 0; j < newCount; j++) {
            ...
            const angle = firstLocalMod + (2 * Math.PI * j) / newCount;
            const axisRes = evolu.insert('axis', {
          ...
          for (const template of firstPlaces) {
            ...
            const baseName =
              (templateWithName.name?.trim().match(/^(.+?) Echo \d+$/) ??
                null)?.[1] ??
            ...
            for (let idx = 0; idx < newAxisIds.length; idx++) {
              ...
              for (const rep of repeatersAtTemplate) {
                const repEchoGroupId =
                  (rep as { repeaterEchoGroupId?: CircularRepeaterId })
                    .repeaterEchoGroupId ?? rep.id;
                ...
                if (newRepRes.ok) {
                  ...
                  for (let k = 0; k < Number(rep.count); k++) {
                    const axisAngle =
                      firstLocalEcho + (2 * Math.PI * k) / Number(rep.count);
                    evolu.insert('axis', {
                      placeId: newEchoId,
                      angle: axisAngle,
                      isBidirectional: 0,
          ...
          for (let k = 0; k < toKeep.length; k++) {
            const angle = firstLocalMod + (2 * Math.PI * k) / newCount;
            const ax = toKeep[k];
            if (ax) evolu.update('axis', { id: ax.id, angle });
          }
          ...
          for (const [, groupPlaces] of byGroup) {
            ...
            const baseName =
              (sorted[0]?.name?.trim().match(/^(.+?) Echo \d+$/) ??
                null)?.[1] ??
              sorted[0]?.name?.trim() ??
              'Place';
            sorted.forEach((p, i) => {
            ...
            if (pa && transformChoice() === 'addPlaceOnCircularRepeater') {
              setPendingAddPlaceOnCircularRepeater({
                ...
                ...(enabled && {
                  ...
                  alternatingStart:
                    pa.alternatingStart ?? pa.placementAxisNumber ?? 1,
                }),
              });
            }
          ...
          axisDirection={
            ...
            (pendingAddAxis() ?? pendingModifyAxis())
              ? {
                  ...
                  isMirror:
                    (pendingAddAxis() ?? pendingModifyAxis())?.isMirror ??
                    false,
          ...
          >
            {transformChoice() === 'rotate' &&
              ...
              (() => {
                ...
                const groupId = (selectedPlace as PlaceLike)
                  .repeaterEchoGroupId;
                const placesToDraw =
                  groupId != null
                    ? placesAbs.filter(
                  ...
                  <g>
                    {placesToDraw.map((place) => {
                      ...
                      const dirY = isAxisPlace
                        ? Math.sin(theta)
                        : -Math.cos(theta);
                      const ex = cx + ORIENTATION_AXIS_LENGTH * dirX;
                      const ey = cy + ORIENTATION_AXIS_LENGTH * dirY;
                      ...
                      return (
                        // @ts-expect-error Solid reconciliation key on SVG group
                        <g key={place.id}>
                          <line
                            x1={cx}
                            y1={cy}
            ...
            {transformChoice() === 'rotate' &&
              ...
              selectedCircularRepeaterId() &&
              (() => {
                const repId = selectedCircularRepeaterId();
                if (repId == null) return null;
                const axesList = axes();
                const placesAbs = placesWithAbsolutePositions();
                const hubRow = circularRepeaters().find((r) => r.id === repId);
                const hub = hubRow
                  ? placesAbs.find((p) => p.id === hubRow.placeId)
                  : null;
                if (!hub) return null;
                const refWorld =
                  getRepeaterReferenceAxisWorldAngle(
                ...
                const prRot = pendingRotate();
                const theta =
                  prRot?.kind === 'circularRepeater' &&
                  prRot.circularRepeaterId === repId
                    ? prRot.angle
                    : refWorld;
                ...
                const cursor = lastCursorCanvas();
                const tRaw = cursor
                  ? projectPointOntoAxis(cursor.x, cursor.y, cx, cy, axisAngle)
                  : 0.8 * ORIENTATION_AXIS_LENGTH;
                const t = Math.max(0, Math.min(ORIENTATION_AXIS_LENGTH, tRaw));
                const handleX = cx + t * dirX;
                const handleY = cy + t * dirY;
                const arrowSize = 18;
                const baseX = ex - arrowSize * dirX;
            ...
            {(() => {
              ...
              const showAxisHandle =
                ...
                (tc === 'addAxis' || tc === 'modifyAxis') &&
                (pa != null || pmod != null);
              const prRep = pendingRotate();
              const axesListRaw = axes();
              const placesListRot = places();
              const deltaRepeaterPreview =
                prRep?.kind === 'circularRepeater' &&
                tc === 'rotate' &&
                gs === 'execute'
                  ? (() => {
                      const refB = getRepeaterReferenceAxisWorldAngle(
                        prRep.circularRepeaterId,
                        axesListRaw,
                        placesAbs,
                      );
                      return refB != null ? prRep.angle - refB : null;
                    })()
                  : null;
              const repeaterRotateSigns =
                prRep?.kind === 'circularRepeater' &&
                tc === 'rotate' &&
                gs === 'execute' &&
                deltaRepeaterPreview != null
                  ? rotationSignByRepeaterId(
                      getCircularRepeaterEchoGroupIds(
                        prRep.circularRepeaterId,
                        circularRepeaters(),
                        placesListRot,
                        axesListRaw,
              ...
              const axesToDraw: Array<{
                ...
                if (
                  rid != null &&
                  deltaRepeaterPreview != null &&
                  repeaterRotateSigns != null
                ) {
                  const s = repeaterRotateSigns.get(rid);
                  if (s != null) ang += s * deltaRepeaterPreview;
                }
                return {
                  id: axis.id,
                  placeId: axis.placeId,
                  ...
                  <g>
                    ...
                    {isRepeaterSelected &&
                      ...
                      (() => {
                        ...
                        const distFromAxis =
                          svgTokens.axisLabelDistanceFromAxis;
                        const cosA = Math.cos(geom.worldAngle);
                        const sinA = Math.sin(geom.worldAngle);
                        const labelX =
                          geom.originX +
                        ...
                        return (
                          <text
                            ...
                            font-size={String(svgTokens.axisLabelFontSize)}
            ...
            {(() => {
              ...
              const dragPos = radiusHandleDragPosition();
              const angle = pmc.radiusHandleAngle ?? cf.radiusHandleAngle ?? 0;
              const handleX =
                isDraggingThis && dragPos
                  ? dragPos.x
